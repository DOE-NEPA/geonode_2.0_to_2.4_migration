...
src_cur.execute("select id, name, slug from taggit_tag")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into taggit_tag(id, name, slug) values (%s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, name, slug from taggit_tag"
    print str(src_row)
    dst.rollback()
