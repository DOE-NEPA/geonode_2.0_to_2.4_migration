...
src_cur.execute("select id, pickled_data from notification_noticequeuebatch")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into notification_noticequeuebatch(id, pickled_data) values (%s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, pickled_data from notification_noticequeuebatch"
    print str(src_row)
    dst.rollback()
