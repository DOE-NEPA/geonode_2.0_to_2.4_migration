...
src_cur.execute("select id, thread_id, sender_id, sent_at, content from user_messages_message")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into user_messages_message(id, thread_id, sender_id, sent_at, content) values (%s, %s, %s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, thread_id, sender_id, sent_at, content from user_messages_message"
    print str(src_row)
    dst.rollback()
