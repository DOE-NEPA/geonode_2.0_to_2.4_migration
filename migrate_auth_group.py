...
src_cur.execute("select id, name from auth_group")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into auth_group(id, name) values (%s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, name from auth_group"
    print str(src_row)
    dst.rollback()
