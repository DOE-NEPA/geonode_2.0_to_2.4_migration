...
src_cur.execute("select id, upload_id, file, slug from upload_uploadfile")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into upload_uploadfile(id, upload_id, file, slug) values (%s, %s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, upload_id, file, slug from upload_uploadfile"
    print str(src_row)
    dst.rollback()
