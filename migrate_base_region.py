...
src_cur.execute("select id, code, name from base_region")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into base_region(id, code, name, name_en, parent_id, lft, rght, tree_id, level) values (%s, %s, %s, %s, %s, %s, %s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, code, name from base_region"
    print str(src_row)
    dst.rollback()
