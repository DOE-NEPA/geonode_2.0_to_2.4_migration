...
src_cur.execute("select id, subject from user_messages_thread")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into user_messages_thread(id, subject) values (%s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, subject from user_messages_thread"
    print str(src_row)
    dst.rollback()
