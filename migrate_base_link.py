...
src_cur.execute("select id, resource_id, extension, link_type, name, mime, url from base_link")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into base_link(id, resource_id, extension, link_type, name, mime, url) values (%s, %s, %s, %s, %s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, resource_id, extension, link_type, name, mime, url from base_link"
    print str(src_row)
    dst.rollback()
