...
src_cur.execute("select id, layer_id, style_id from layers_layer_styles")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into layers_layer_styles(id, layer_id, style_id) values (%s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, layer_id, style_id from layers_layer_styles"
    print str(src_row)
    dst.rollback()
