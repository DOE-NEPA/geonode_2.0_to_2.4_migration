...
src_cur.execute("select id, user_id, timezone, language from account_account")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into account_account(id, user_id, timezone, language) values (%s, %s, %s, %s)", assignments)
    dst.commit()
