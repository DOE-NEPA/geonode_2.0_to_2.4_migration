...
src_cur.execute("select id, label, display, description, \"default\" from notification_noticetype")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into notification_noticetype(id, label, display, description, \"default\") values (%s, %s, %s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, label, display, description, \"default\" from notification_noticetype"
    print str(src_row)
    dst.rollback()
