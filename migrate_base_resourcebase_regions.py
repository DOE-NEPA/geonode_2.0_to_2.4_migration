...
src_cur.execute("select id, resourcebase_id, region_id from base_resourcebase_regions")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into base_resourcebase_regions(id, resourcebase_id, region_id) values (%s, %s, %s)", assignments)
    dst.commit()
  except Exception as error:
    print 
    print type(error)
    print str(error) + "select id, resourcebase_id, region_id from base_resourcebase_regions"
    print str(src_row)
    dst.rollback()
