...
src_cur.execute("select id, user_id, email, verified, \"primary\" from account_emailaddress")
...
for src_row in src_cur:
  ...
  try:
    dst_cur.execute("insert into account_emailaddress(id, user_id, email, verified, \"primary\") values (%s, %s, %s, %s, %s)", assignments)
    dst.commit()
